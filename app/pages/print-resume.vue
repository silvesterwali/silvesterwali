<template>
  <UContainer class="p-5 bg-white text-black text-xs leading-snug font-sans print:p-0 print:max-w-full">
    ...
    <!-- Skills -->
    <section class="mb-3">
      <h2 class="text-sm font-bold uppercase border-b-2 border-gray-900 pb-1 mb-2 text-gray-900">Skills</h2>
      <p class="m-0 text-xs text-gray-800">{{ skills.join(', ') }}</p>
    </section>

    <!-- Work Experience -->
    <section class="mb-3">
      <h2 class="text-sm font-bold uppercase border-b-2 border-gray-900 pb-1 mb-2 text-gray-900">Work Experience</h2>
      <div class="flex flex-col gap-4">
        ...
        >
          ...
          <p
            v-if="exp.stack && exp.stack.length"
            class="text-[10px] m-0 text-gray-700"
          >
    ...
    <!-- Certificates -->
    <section class="mb-3">
      <h2 class="text-sm font-bold uppercase border-b-2 border-gray-900 pb-1 mb-2 text-gray-900">Certificates</h2>
      <div class="flex flex-col gap-1">
        <div
          v-for="cert in certificates"
          :key="cert.title"
          class="text-[11px] text-gray-800"
        >
          <span class="font-bold text-gray-900">{{ cert.title }}</span> — {{ cert.issuer }}, {{ cert.issueDate }}
        </div>
      </div>
    </section>
  </UContainer>
